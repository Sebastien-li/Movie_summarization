...
dataset_dir = dir_path + '/test_movies/10_movies/'
...
def get_ilp_alignment(output_dir, log_file, code, similarity_matrix, limit_candidate=True, percentile=50):
	...
	model.addConstr(x.sum() >= lowerbound)				
	model.addConstr(x.sum() <= upperbound)			
...
if __name__ == "__main__":  
	if len(sys.argv) > 3 and sys.argv[1] == 'scriptbase':
		...
		run_experiment('auto_alignment_bm25.w2v.sts', 1, top_n=False, sim_matrix='bm25.w2v', compute_similarity=True, run_alignment=True)
